...
export function HeaderBar({
  ...
  const effectiveStatus = isUsingMockData ? "OFFLINE" : systemStatus;
  const status = statusMap[effectiveStatus];
    ...
    <header className="relative overflow-hidden rounded-[28px] border border-white/10 bg-slate-900/45 px-5 py-6 shadow-[0_0_45px_rgba(15,23,42,0.5)] backdrop-blur-xl sm:px-6 lg:px-8">
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl space-y-3">
          ...
          <div>
            ...
            <p className="mt-2 text-lg font-medium tracking-[0.18em] text-sky-100">
              EPNI YAŞAM DESTEK KAPSÜL PANELİ
            </p>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
              Gaz, Toprak Nemi, Sıcaklık ve Yağmur verilerini anlık izleyen
              takım kontrol paneli.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-stretch">
          <div
            className={`rounded-2xl bg-slate-950/50 px-4 py-4 backdrop-blur-md ${
              effectiveStatus === "OFFLINE"
                ? "border border-rose-400/20 shadow-[0_0_30px_rgba(251,113,133,0.1)]"
                : "border border-emerald-400/20 shadow-[0_0_30px_rgba(16,185,129,0.1)]"
            }`}
          >
